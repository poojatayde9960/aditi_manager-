...
import { MapPin, Calendar, CheckCircle, Clock, Truck, X, ChevronUp } from 'lucide-react';
...
export default function CustomerDetails() {
  ...
  const { data: giftData, isLoading: isGiftLoading } = useGiftGetByIdQuery(undefined, {
    ...
    <>
      ...
      <div className="min-h-screen bg-[#020523] text-white">
        <div className="max-w-10xl mx-auto">
          ...
          {/* Order Card (shown only if current order status matches the active tab) */}
          {(() => {
            const isCompleted = ['completed', 'delivered'].includes(orderStatus.toLowerCase());
            const shouldShow = (activeTab === 'Ongoing Orders' && !isCompleted) ||
              (activeTab === 'Completed Orders' && isCompleted);

            if (!shouldShow && (activeTab === 'Ongoing Orders' || activeTab === 'Completed Orders')) {
              return (
                <div className="bg-[#00D4FF0F] backdrop-blur-sm rounded-2xl p-10 border border-slate-700/50 text-center text-slate-400">
                  No {activeTab.toLowerCase()} found for this specific view
                </div>
              );
            }
                ...
                <div className="bg-[#00D4FF0F] backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <ChevronUp
                        size={22}
                        className="cursor-pointer text-slate-400 hover:text-white transition-colors ml-2"
                        onClick={() => navigate(-1)}
                      />
                    </div>
                  </div>
                          ...
                          <div className="flex items-center justify-between rounded-xl p-4">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 bg-[#FFFFFF12] rounded-lg flex items-center justify-center">
                                <img
                                  src={item.productId?.productDetailsSection?.productImages?.[0] || item.productId?.bottleImg || img}
                                  className="h-10 w-8 object-contain"
                                  alt={item.productId?.heroSection?.productName || item.productId?.name}
                                />
                              </div>
                              <div>
                                <h5 className="font-medium">
                                  {item.productId?.heroSection?.productName || item.productId?.name || 'Product'}
                                </h5>
                                <p className="text-slate-400 text-sm">Quantity: {item.quantity}</p>
                              </div>
                            </div>
                            <div className="text-[#22FF00] font-semibold">
                              €{(item.productId?.productDetailsSection?.price || item.productId?.price || 0).toLocaleString()}
                            </div>
                          </div>
                  ...
                  {/* Shipping Address */}
                  <div className="mb-6 bg-[#020523] rounded-xl p-6 mt-6">
